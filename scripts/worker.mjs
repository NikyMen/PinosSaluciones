...
const uri=process.env.MONGODB_URI;const interval=Math.max(60000,Number(process.env.WORKER_INTERVAL_MS||300000));
...
await mongoose.connect(uri,{maxPoolSize:5});await run();setInterval(()=>run().catch(error=>console.error("[worker]",error)),interval);
