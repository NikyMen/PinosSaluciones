...
export async function POST(request: Request) {
  try {
    ...
    await connectDB();
    const { supplierId, items, workId, expectedDate, notes } = parsed.data;
    const result = await createPurchaseOrder({ supplierId, lines: items, workId, expectedDate, notes, session });
