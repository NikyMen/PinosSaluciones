...
export const schemas: Record<Entity, z.ZodObject<z.ZodRawShape>> = {
  ...
  workers: z.object({ firstName: text, lastName: text, dni: z.string().trim().transform(v => v.replace(/\D/g, "")).refine(v => v.length >= 7 && v.length <= 8, "El DNI tiene que tener 7 u 8 dígitos"), phone: optionalText, category: z.enum(["capataz", "oficial", "medio_oficial", "ayudante", "especialista"]), dailyRateCents: cents.default(0), hoursPerDay: z.coerce.number().min(1).max(24).default(8), notes: optionalText, active: z.boolean().optional().default(true) }),
